...
func (p *CalculatorHandler) Calculate(logid int32, w *rpc.Work) (val int32, err error) {
	fmt.Print("calculate(", logid, ", {", w.Op, ",", w.Num1, ",", w.Num2, "})\n")
	switch w.Op {
	case rpc.Operation_ADD:
		val = w.Num1 + w.Num2
		break
	case rpc.Operation_SUBTRACT:
		val = w.Num1 - w.Num2
		break
	case rpc.Operation_MULTIPLY:
		val = w.Num1 * w.Num2
		break
	case rpc.Operation_DIVIDE:
		if w.Num2 == 0 {
			ouch := rpc.NewInvalidOperation()
